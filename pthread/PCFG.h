...
struct ThreadData {
    segment* seg_ptr;       // 指向segment的指针
    int start_idx;          // 起始索引
    int end_idx;            // 结束索引
    vector<string>* guesses_ptr; // 指向猜测结果容器的指针
    string* prefix_ptr;     // 前缀字符串指针(用于第二种情况)
    int* total_guesses_ptr; // 总猜测数指针
    pthread_mutex_t* mutex_ptr; // 互斥锁指针
};
// 优先队列，用于按照概率降序生成口令猜测
...
public:
    ...
    // 添加线程工作函数声明
    static void* ProcessSegmentValues(void* arg);
