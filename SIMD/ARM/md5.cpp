...
void MD5Hash_NEON(string inputs[4], bit32 states[4][4]) {
    ...
    // 初始化4个MD5状态（四路并行）
    uint32x4_t state[4];
    state[0] = vdupq_n_u32(0x67452301); // a
    state[1] = vdupq_n_u32(0xefcdab89); // b
    state[2] = vdupq_n_u32(0x98badcfe); // c
    state[3] = vdupq_n_u32(0x10325476); // d
	...
	for (int i = 0; i < 4; i++) {
        uint32_t value = vgetq_lane_u32(state[i], 0);
        states[0][i] = ((value & 0xff) << 24) | ((value & 0xff00) << 8) | 
                           ((value & 0xff0000) >> 8) | ((value & 0xff000000) >> 24);
        
        value = vgetq_lane_u32(state[i], 1);
        states[1][i] = ((value & 0xff) << 24) | ((value & 0xff00) << 8) | 
                           ((value & 0xff0000) >> 8) | ((value & 0xff000000) >> 24);
        
        value = vgetq_lane_u32(state[i], 2);
        states[2][i] = ((value & 0xff) << 24) | ((value & 0xff00) << 8) | 
                           ((value & 0xff0000) >> 8) | ((value & 0xff000000) >> 24);
        
        value = vgetq_lane_u32(state[i], 3);
        states[3][i] = ((value & 0xff) << 24) | ((value & 0xff00) << 8) | 
                           ((value & 0xff0000) >> 8) | ((value & 0xff000000) >> 24);
    }
    // 释放内存
    for (int i = 0; i < 4; i++) {
