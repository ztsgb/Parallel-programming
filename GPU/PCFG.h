...
#include <queue>
#include <omp.h>
#include <cuda_runtime.h> // 修复cudaMalloc/cudaFree未声明
// #include <chrono>   
// using namespace chrono;
using namespace std;

// CUDA相关常量定义，防止未声明错误
#ifndef MAX_GUESS_LENGTH
#define MAX_GUESS_LENGTH 128
#endif
#ifndef MAX_PREFIX_LENGTH
...
public:
    ...
    char* d_all_values = nullptr;       // 设备端字符串池
    int* d_offsets = nullptr;           // 设备端偏移数组
    int* d_lengths = nullptr;           // 设备端长度数组
    char* d_output = nullptr;           // 设备端输出缓冲区
    char* d_prefix = nullptr;           // 设备端前缀缓存
    size_t current_values_size = 0;     // 当前值缓冲区大小
    size_t current_output_size = 0;     // 当前输出缓冲区大小
    
    // 初始化CUDA资源
    void InitCUDAResources(size_t max_values_size, size_t max_output_size) {
        cudaMalloc(&d_all_values, max_values_size);
        cudaMalloc(&d_offsets, max_output_size * sizeof(int));
        cudaMalloc(&d_lengths, max_output_size * sizeof(int));
        cudaMalloc(&d_output, max_output_size * MAX_GUESS_LENGTH);
        cudaMalloc(&d_prefix, MAX_PREFIX_LENGTH);
        current_values_size = max_values_size;
        current_output_size = max_output_size;
    }
    
    // 释放CUDA资源
    void FreeCUDAResources() {
        if(d_all_values) cudaFree(d_all_values);
